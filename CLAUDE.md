...
- Pushing to main auto-deploys; ~1-3 min. Confirm served SHA after deploy via
  `/api/health/db` -> `vercel_git_commit_sha`, and job-board freshness via
  `/api/health` -> `jobs_freshness` (last row seen, last completed cycle).
- The For You digest (section 14) shipped in `14da449` and has served from
  production Postgres since 2026-08-31, which supersedes that section's "NOT
  VERIFIED ON POSTGRES" note.

FUNCTIONS RUN IN sin1. vercel.json `regions: ["sin1"]` puts them next to the
Supabase pooler (ap-southeast-1). Until 2026-09-11 they ran in the default
iad1 (US East): every query crossed the Pacific and `/api/health/db` measured
3.3s. Most "X loads slowly" reports traced back to this, not to the queries.

THE SYNC NO LONGER DEPENDS ON CRON_SECRET. It refused outright without it,
which is how the corpus froze (next paragraph). Now a correct bearer runs
immediately and anything else is THROTTLED (ats.sync_allowed_unauthenticated):
240s between anonymous runs, and a 3h rest after a completed cycle unless a
cycle is part-way through. Six daily Vercel cron entries drive it. A full
cycle (319 companies, 68 of them Workday) is ~4-5 minutes of fetching, so it
spans several 50s runs; the per-company cursor makes that safe.

WORKDAY IS THE EIGHTH PLATFORM (ats.py `_wd_*`, registry generated by
scratch/add_workday_registry.py). Keyless CXS endpoint, POST, 20 rows a page,
dates only as "Posted N Days Ago" (converted at fetch; "30+" skipped rather
than guessed), no description in the list (fetched on open). Pages are fetched
in parallel after the first -- sequential paging made one pass take most of an
hour. Five employers that answered (CVS, Lowe's, Home Depot, T-Mobile, AT&T)
were left out: overwhelmingly store and driver roles that would drown the board.

AI ROUTING prefers MEASURED quality among models that can fit a request
(`prio` in ai.py), headroom only breaks ties. groq/compound-mini and
gemini-flash-lite-latest return HOLLOW analyses (valid JSON, no score, no
dimensions) -- they carry max_budget=4000 so they never get the analysis call,
and analysis.py rejects hollow output (_is_substantive) and retries on a
DIFFERENT model (ai.excluding). The analysis asks 4500 tokens: prompt + 6000
exceeded gpt-oss's whole 8000 TPM bucket, so the router used to skip them.

VERCEL ENV VARS ARE BAKED IN AT DEPLOY TIME. Setting one in the dashboard does
NOT affect the running functions -- the deployment must be re-created before
`os.environ` sees it. This cost a full round trip on CRON_SECRET (2026-09-08:
value added, `/api/health` still reported `cron_secret: false`, because no new
deployment had been made). After changing any env var, redeploy and then
confirm the change through `/api/health`, which reports presence booleans for
CRON_SECRET and SECRET_KEY precisely so this is checkable from outside.

THE JOB CORPUS FREEZES SILENTLY. `ats_jobs` had not been refreshed since
2026-08-27 (17,979 rows, every `last_seen_at` identical) because the nightly
cron was answering 503 on an unset CRON_SECRET. Nothing surfaces this to a
visitor: Browse and For You keep serving the stale corpus, and `prune_stale()`
then DELETES anything unseen for 21 days, so the failure mode is an empty job
board roughly three weeks later rather than an error anyone sees. Check
freshness with `SELECT count(*), max(last_seen_at) FROM ats_jobs` whenever the
job layer is touched.

CONNECTED TOOLING BEATS THE LOCAL `.env`. The local `.env` has no
SUPABASE_DB_URL, so the Postgres-only scripts in section 9 cannot run as
written -- but the Supabase MCP server reaches the same production database
directly, and the Vercel MCP server reports deployments, so both "verify
against real Postgres" and "confirm what is actually deployed" are reachable
